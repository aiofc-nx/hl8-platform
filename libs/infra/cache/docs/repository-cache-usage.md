...
const cachedRepo = new CachedRepository(
  'User',
  cache,
  logger,
  cacheConfig,
...
class CustomCachedRepository<T extends Entity> extends CachedRepository<T> {
  protected buildCacheKey(id: string, options?: { tenantId?: string }): string {
    ...
    return tenant 
      ? `${tenant}:custom:${this.entityName}:${id}`
      : `custom:${this.entityName}:${id}`;
  }
}
```
...
const users = await Promise.all([
  userRepo.findById(new EntityId('1')),
  userRepo.findById(new EntityId('2')),
  userRepo.findById(new EntityId('3')),
]);
