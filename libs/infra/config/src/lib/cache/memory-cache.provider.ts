...
export class MemoryCacheProvider implements CacheProvider {
  ...
  private async evictLeastRecentlyUsed(): Promise<void> {
    ...
    if (this.options.maxSize) {
      const toRemove = Math.max(0, entries.length - this.options.maxSize + 1);
  ...
  private calculateSize(value: ConfigRecord): number {
    try {
      return JSON.stringify(value).length; // 使用 UTF-8 编码长度
    } catch {
      return 0;
    }
  }
  ...
  private updateAverageAccessTime(accessTime: number): void {
    ...
    if (totalHits === 1) {
      ...
      this.stats.averageAccessTime =
        (this.stats.averageAccessTime * (totalHits - 1) + accessTime) /
        totalHits;
    }
  }
