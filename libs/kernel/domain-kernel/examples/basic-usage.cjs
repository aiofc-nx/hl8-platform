...
class UserProfile extends InternalEntity {
  constructor(aggregateRootId, email, name, id, auditInfo, lifecycleState, version) {
  ...
  setEmail(newEmail) {
    ...
    this.updateState({ email: newEmail, name: this._name }, this.auditInfo.updatedBy);
  }

  setName(newName) {
    this._name = newName;
    this.updateState({ email: this._email, name: newName }, this.auditInfo.updatedBy);
  }
  ...
  performBusinessOperation(params) {
    return this.executeBusinessLogic(params);
  }

  performStateUpdate(newState) {
    this._email = newState.email;
    this._name = newState.name;
  }
...
class User extends AggregateRoot {
  constructor(email, name, id, auditInfo, lifecycleState, version) {
    ...
    this._email = email;
    this._name = name;
    this._profile = new UserProfile(id, email, name, new EntityId(), auditInfo, EntityLifecycle.CREATED, 1);
    this._profile.activate(); // 激活内部实体
