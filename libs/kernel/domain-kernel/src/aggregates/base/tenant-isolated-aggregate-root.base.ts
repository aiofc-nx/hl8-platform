/**
 * @fileoverview 租户隔离聚合根基类
 * @description 支持租户隔离的聚合根基类，扩展自 AggregateRoot 基类
 */

import { AggregateRoot, type DomainEvent } from "./aggregate-root.base.js";
...
/**
 * 租户隔离聚合根基类
 * @description 提供租户、组织和部门级别的数据隔离能力，并自动将租户信息添加到领域事件
 */
export abstract class TenantIsolatedAggregateRoot extends AggregateRoot {
  ...
  public override addDomainEvent(event: DomainEvent): void {
    ...
    super.addDomainEvent(enrichedEvent);
  }
  ...
  /**
   * 比较两个聚合根是否相等（包含租户隔离信息）
   * @param other 要比较的另一个聚合根
   * @returns 是否相等
   * @override
   */
  public override equals(other: AggregateRoot | null | undefined): boolean {
    if (!super.equals(other)) {
      return false;
    }

    if (!(other instanceof TenantIsolatedAggregateRoot)) {
      return false;
    }
